...
def sha224(M, form = "hex"):
    ...
    H = [0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7,0xbefa4fa4]
                                  # NIST document page 22, section 6.2.2-2:                                                             # intialise hash.
    ...
    if(form=="bin"):
        return ''.join(format(h, '032b') for h in H)[:224]         # convert final hash values into binary string.

    return ''.join(format(h, '08x') for h in H)[:56]              # convert final hash values into hex string.
